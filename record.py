...
def run_ffmpeg(url):
    ...
    # tunggu file dibuat sebelum upload
    wait_seconds = 0
    while not os.path.exists(filename) and wait_seconds < 30:
        time.sleep(1)
        wait_seconds += 1
    ...
    if os.path.exists(filename) and os.path.getsize(filename) > 0:
        ...
        print(f"\n[ ERROR ] File rekaman {filename} tidak ditemukan atau kosong, upload dibatalkan.")
